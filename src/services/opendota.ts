import type { OpenDotaHero, OpenDotaMatch } from '@/types/opendota';

export { type OpenDotaPlayer } from '@/types/opendota';

const OPENDOTA_BASE = 'https://api.opendota.com/api',
  RETRY_DELAYS_MS = [2000, 5000, 10_000],
  REQUEST_TIMEOUT_MS = 10_000,
  heroCache = new Map<number, string>(),
  profileCache = new Map<number, string>();
let heroesPromise: Promise<void> | null = null;

const PARSE_REQUEST_TTL_MS = 5 * 60_000,
  parseRequested = new Map<string, { expiresAt: number; promise: Promise<void> }>();
...
function ensureHeroesLoaded(): Promise<void> | undefined {
  ...
  if (!heroesPromise) {
    heroesPromise = loadHeroes().catch((error) => {
      heroesPromise = null;
      throw error;
    });
  }
...
export async function fetchPlayerProfile(accountId: number): Promise<string | null> {
  if (profileCache.has(accountId)) {
    return profileCache.get(accountId)!;
  }
  try {
    ...
    if (name) {
      profileCache.set(accountId, name);
    }
    return name ?? null;
  } catch {
    return null;
...
export async function requestParse(matchId: string, logger: DebugLogger): Promise<void> {
  const now = Date.now(),
    existing = parseRequested.get(matchId);
  if (existing) {
  ...
  const entry = { expiresAt: Number.POSITIVE_INFINITY, promise: Promise.resolve() };
  const promise = (async () => {
    ...
    try {
      ...
      entry.expiresAt = Date.now() + PARSE_REQUEST_TTL_MS;
      parseRequested.set(matchId, entry);
      debugLog(logger, 'parse_request_succeeded', { match_id: matchId });
    } catch (error) {
      if (parseRequested.get(matchId)?.promise === entry.promise) {
        parseRequested.delete(matchId);
      }
      const msg = error instanceof Error ? error.message : String(error);
      debugLog(logger, 'parse_request_failed', { match_id: matchId, error: msg });
      throw Object.assign(new Error(`opendota_parse_request_failed: ${msg}`), { cause: error });
    }
  })();
...
export async function fetchMatch(matchId: string, logger: DebugLogger): Promise<OpenDotaMatch | null> {
  ...
  for (let attempt = 0; attempt < RETRY_DELAYS_MS.length; attempt++) {
    ...
    try {
      ...
      for (let retryAttempt = 0; retryAttempt < RETRY_DELAYS_MS.length; retryAttempt++) {
        ...
        const parsedMatch = await (async () => {
          try {
            ...
            return await fetchMatchJson(matchId, attempt + 1, true, logger, retryAttempt + 1);
          } catch (error) {
            const msg = error instanceof Error ? error.message : String(error);
        ...
        if (!isIncompleteMatch(parsedMatch)) {
          return parsedMatch;
        }
      }
      ...
      const msg = error instanceof Error ? error.message : String(error),
        isHttp404 = msg.startsWith('opendota_http_404'),
        isLast = attempt === RETRY_DELAYS_MS.length - 1;
      if (!isHttp404) {
        throw error;
      }
      if (isLast) {
...
async function fetchMatchJson(
  ...
  try {
    ...
    if (error instanceof Error && error.message.startsWith('opendota_http_')) {
      throw error;
    }
