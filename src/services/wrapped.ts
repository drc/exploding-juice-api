...
export class WrappedLogic {
	...
	static detectPersonalityBadge(stats: Tier1Stats & Tier2Stats): PersonalityBadge {
		...
		const assistsPerMatch = stats.total_assists / Math.max(1, stats.total_kills + stats.total_deaths);
	 ...
	 */
	static async findThatOneGame(accountId: number, weekStartDate: string): Promise<GameAnomaly | null> {
