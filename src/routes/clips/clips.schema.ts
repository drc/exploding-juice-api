...
export const clipRequestSchema = z
  .object({
    url: z.string().url().openapi({ examples: ["https://www.youtube.com/watch?v=dQw4w9WgXcQ"] }),
    start: z.number().int().min(0).openapi({ examples: [0] }),
    end: z.number().int().positive().openapi({ examples: [10] }),
    name: z
      .string()
      .min(1)
      .max(80)
